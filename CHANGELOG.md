...
- Documented MCP tool guidance and allowed-role discovery metadata in `tools/list`.
...
- Documented compact topology for filtered graphs with at least 24 nodes or 30 edges, bounded endpoint components, related-node drill-down, raw relationship fallback, and safe topology-cap behavior.
...
- Documented read-only plugin and binding metadata views, exact-version plugin detail pages, plugin-aware topology, and the separation between plugin metadata and the Tools inventory.
