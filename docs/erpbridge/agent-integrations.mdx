...
OpenCode uses mcp.servers in opencode.json or opencode.jsonc.
...
OpenCode uses the env:NAME substitution for environment-backed values. It does
not document a server-native tool allowlist in this config shape; restrict
exposure in the ERPBridge registry and use approval controls for writes.
Restart the OpenCode session after changing the config.
