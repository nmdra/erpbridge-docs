...
reference](./tool-schema.mdx) for the legacy fallback and protected-header
...
non-secret API definition, `credentialRef`, and optional `credentialSource`
(`env` by default or `file`). File mode requires `ERPBRIDGE_CREDENTIALS_DIR`,
reads the reference-named file for each probe, and never falls back to the
environment. The server resolves the credential and applies its ERP transport
policy. It returns only status, normalized content type, latency, and success.
