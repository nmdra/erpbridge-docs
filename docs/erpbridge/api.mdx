...
`http://mock-erp:8081`. The matching OpenAPI contract is published at
...
active exact plugin and tool versions. Use resource-specific name and version
filters to list exact records.
...
The plugin returns `{"result": <JSON value>}`. Request and response JSON are
limited to 1 MiB; redirects and retries are disabled. A timeout, non-2xx
response, malformed or oversized response, or invalid transformed output follows
the binding policy: `continue` keeps the original result and `fail` returns a
generic tool error. Plugin URLs, payloads, credentials, and plugin error bodies
are not returned to callers.
...
misses; the cache stores the final transformed MCP result. Plugin and binding
lifecycle changes flush affected target-tool cache entries.
