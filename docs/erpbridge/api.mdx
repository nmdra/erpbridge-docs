...
Body: one JSON tool definition (kind `MCPTool`). Returns `201 Created` when a
new definition is stored and reports an update when an existing version is
replaced. The `bridgectl tool apply` command also accepts a YAML sequence or
...
non-secret API definition and `credentialRef`; the server resolves the
credential and applies its ERP transport policy. It returns only status,
normalized content type, latency, and success. ERP response bodies and upstream
headers never cross the probe boundary. `bridgectl api test` uses this route by
default; `--local` is an explicit legacy host-side diagnostic.

For cache endpoints, `503` means the cache is not enabled.
