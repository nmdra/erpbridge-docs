...
ERPBridge can pass a successful, normalized tool result to an externally
operated HTTP plugin. The control plane stores the endpoint and exact release,
but never installs, starts, upgrades, or schedules plugin code.

A `PluginBinding` connects one exact plugin version to one exact tool version.
Active `after_response` bindings run synchronously in ascending priority order
before MCP or direct-invoke serialization. They do not add tools to `tools/list`.
The plugin request contains only the protocol version, invocation ID, exact tool
identity, normalized result, and binding configuration. It excludes original
arguments, inbound headers, caller identity, caller tokens, and ERP credentials.
...
MCP result, and plugin or binding lifecycle changes flush affected tool cache
entries. `continue` keeps the original result after a plugin failure; `fail`
returns a generic tool error.
...
`PLUGIN_ENDPOINT_ALLOWLIST`. Redirects, URL userinfo, raw secrets, and broad
...
- **Secret Decoupling**: Schemas and plugin resources never contain raw tokens or keys. They only contain a `credentialRef`; the middleware resolves it at request time from the environment. Legacy local API registries require an explicit confirmed scrub before state changes continue.
