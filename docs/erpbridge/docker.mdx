...
ERP credentials or inbound request headers.

The ERPBridge repository includes an opt-in black-box fixture. It builds the
`mock-plugin` from the sibling `../ERPBridge-Plugins` polyrepo, starts it with
the pinned MockERP image, tests both MCP and direct invocation, and removes only
