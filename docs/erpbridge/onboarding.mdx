...
The command writes one JSON schema file per generated tool into `schemas/erp/` (one directory per module), for example `schemas/erp/list_employees.json`. It also prints the generated schemas to stdout — ignore that output; the files on disk are what you apply in the next step.
...
:::tip Why not redirect stdout to a YAML file
The generator writes individual `.json` files to disk for you. If you pipe the stdout output to a file and apply it, `apply` fails — it accepts a single schema or a directory of schema files, not a multi-document manifest.
:::
