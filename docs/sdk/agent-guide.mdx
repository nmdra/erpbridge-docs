...
- Errors are typed: always throw/forward the class hierarchy from `src/types.ts` (`ErpbridgeError` → `AuthenticationError` / `NotFoundError` / `RateLimitError` / `ClientError` (4xx) / `ServerError` (5xx) / `ProtocolError`), never raw `Error` with string-matching.
