...
- Resolve credentials from configuration or environment only — explicit `token`, configured `tokenEnv`, or the default `ERPBridge_TOKEN` env var. Keep token values out of code, logs, tests, and commits.
