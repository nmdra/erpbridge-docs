...
represents a forbidden request and may carry a server-declared `requiredScope`.
...
| `token` | `string` | — | Global bearer token; surface credentials may override it |
| `tokenEnv` | `string` | `ERPBridge_TOKEN` | Environment-variable name for the global bearer token |
